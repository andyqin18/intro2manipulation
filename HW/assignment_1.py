...
def generate_edges(n, phi):
    ...
    cone_edges[:,0] = np.cos(theta) * np.cos(phi)
    cone_edges[:,1] = np.sin(theta) * np.cos(phi)
    cone_edges[:,2] = np.sin(phi)
    # ------------------------------------------------
...
def compute_normals(cone_edges):
    ...
    cone_edges_next = np.vstack([cone_edges[1:], cone_edges[0]])
    facet_normals = np.cross(cone_edges, cone_edges_next)
    # ------------------------------------------------
...
def compute_minimum_distance_from_facet_normals(a, facet_normals):
    ...
    distance = (facet_normals @ a) / lengths 
...
def check_is_interior_point(a, n, phi):
    ...
    is_interior_point = True if compute_minimum_distance(a, n, phi) > 0 else False
    # ------------------------------------------------
